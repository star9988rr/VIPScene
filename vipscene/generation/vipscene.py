...
from vipscene.constants import (
...
from vipscene.generation.ceiling_objects import CeilingObjectGenerator
from vipscene.generation.doors import DoorGenerator
from vipscene.generation.floor_objects import FloorObjectGenerator
from vipscene.generation.layers import map_asset2layer
from vipscene.generation.lights import generate_lights
from vipscene.generation.objaverse_retriever import ObjathorRetriever
from vipscene.generation.object_selector import ObjectSelector
from vipscene.generation.rooms import FloorPlanGenerator
from vipscene.generation.skybox import getSkybox
from vipscene.generation.small_objects import SmallObjectGenerator
from vipscene.generation.utils import get_top_down_frame, room_video
from vipscene.generation.wall_objects import WallObjectGenerator
from vipscene.generation.walls import WallGenerator
from vipscene.generation.windows import WindowGenerator
import json
    ...
    for p in [
        ...
        if not os.path.exists(p):
            raise FileNotFoundError(
                f"Path {p} does not exist, this must exist for generation to succeed."
...
class VIPScene:
    ...
    def get_empty_scene(self):
        return compress_json.load(
            os.path.join(ABS_PATH, "generation/empty_house.json")
        )
    ...
    def empty_house(self, scene):
        ...
        scene["windows"] = []
        scene["objects"] = []
        if "proceduralParameters" not in scene:
            scene["proceduralParameters"] = {}
        scene["proceduralParameters"]["lights"] = []
        return scene

    def generate_rooms(self, scene, plan_json, additional_requirements_room, used_assets=[]):
        self.floor_generator.used_assets = used_assets
        rooms = self.floor_generator.generate_rooms(scene, plan_json, additional_requirements_room)
        scene["rooms"] = rooms
        return scene
    ...
    def generate_windows_from_plan(
        self,
        scene,
        plan_json,
        additional_requirements_window="I want to install windows to only one wall of each room",
        ...
        self.window_generator.used_assets = used_assets
        raw_window_plan, walls, windows = self.window_generator.generate_windows_from_plan(
            scene, plan_json, additional_requirements_window
        )
        scene["raw_window_plan"] = raw_window_plan
        scene["windows"] = windows
        scene["walls"] = walls
    ...
    def select_objects(self, scene, plan_json, additional_requirements_object, used_assets=[]):
        self.object_selector.used_assets = used_assets
        object_selection_plan, selected_objects = self.object_selector.select_objects(
            scene, plan_json, additional_requirements_object
        )
    ...
    def generate_scene(
        ...
        # generate windows
        scene = self.generate_windows_from_plan(
            scene,
            plan_json,
        ...
        scene["floor_objects"] = self.floor_object_generator.generate_objects(
            scene, plan_json, use_constraint=use_constraint
        )
