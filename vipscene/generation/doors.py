...
class DoorGenerator:
    ...
    def generate_doors(self, scene, additional_requirements_door):
        ...
        # Parse the first plan for exterior door
        parsed_plan = None
        for plan in plans:
            temp_parsed = self.parse_door_plan(plan)
            if temp_parsed and (temp_parsed["room_type0"] == "exterior" or temp_parsed["room_type1"] == "exterior"):
                parsed_plan = temp_parsed
                break
        
        if parsed_plan == None:
            # Fallback to default if no plan found
            parsed_plan = {
                "room_type0": rooms[0]["roomType"],
                "room_type1": "exterior",
        ...
        room_type = parsed_plan["room_type0"] if parsed_plan["room_type0"] != "exterior" else parsed_plan["room_type1"]
        current_door["room0"] = room_type
        current_door["room1"] = "exterior"
        current_door["id"] = f"door|0|{room_type}|exterior"

        best_wall_id, interval = find_best_place_for_door(scene)
        if interval == None:
            return raw_doorway_plan, doors, room_pairs, open_room_pairs
            
        connection = self.get_connection_exterior(
            room_type, "exterior", best_wall_id, walls
        )
        ...
        # Force to use doorway for exterior
        connection_type = parsed_plan["connection_type"]
        if connection_type == "open":
            connection_type = "doorway"
        
        # Use parsed plan information instead of hardcoded values
        ...
        door_polygon = self.get_door_polygon(
            connection["segment"], door_dimension, interval, connection_type
        )
    ...
    def get_door_polygon(self, segment, door_dimension, interval, connection_type):
        ...
        if door_width >= original_length:
            ...
            sample_start = min(interval[0], interval[1])
            sample_end = max(interval[0], interval[1])
            door_start = random.uniform(sample_start, sample_end-door_width)
            ...
            door_start, door_end = original_length - door_end, original_length - door_start
            polygon = [
    ...
    def get_connection_exterior(self, room0_id, room1_id, best_wall_id, walls):
        ...
        for interior_wall in interior_walls:
            for exterior_wall in exterior_walls:
                if interior_wall in exterior_wall and best_wall_id in interior_wall:
