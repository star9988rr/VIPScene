...
class ObjathorRetriever:
    ...
    def retrieve(self, queries, threshold=28):
        ...
        sbert_similarities = sbert_similarities * 100
    ...
    def retrieve_by_visual_features(self, visual_feature, threshold=28):
        '''
        Retrieve using visual features instead of text queries
        visual_feature: numpy array of shape (feature_dim,) - the query object's visual feature
        '''
        visual_feature = torch.from_numpy(visual_feature.astype(np.float32))
        visual_feature = F.normalize(visual_feature.unsqueeze(0), p=2, dim=-1)
        
        similarities = 100 * torch.einsum("ij, lkj -> ilk", visual_feature, self.clip_features)
        similarities = torch.max(similarities, dim=-1).values.squeeze(0)
        
        threshold_indices = torch.where(similarities > threshold)
        
        unsorted_results = []
        for asset_index in threshold_indices[0]:
            score = similarities[asset_index].item()
            unsorted_results.append((self.asset_ids[asset_index], score))
    ...
    def retrieve_unified(self, queries=None, visual_feature=None, threshold=28, text_weight=0.0, visual_weight=1.0):
        if queries is None and visual_feature is None:
            raise ValueError("At least one of 'queries' or 'visual_feature' must be provided")
        ...
        with torch.no_grad():
            ...
            if visual_feature is not None:
                visual_feature_tensor = torch.from_numpy(visual_feature.astype(np.float32))
                visual_feature_tensor = F.normalize(visual_feature_tensor.unsqueeze(0), p=2, dim=-1)
                visual_clip_similarities = 100 * torch.einsum(
                    "ij, lkj -> ilk", visual_feature_tensor, self.clip_features
                )
                visual_clip_similarities = torch.max(visual_clip_similarities, dim=-1).values.squeeze(0)
                
                if clip_similarities is not None:
                    total_weight = text_weight + visual_weight
                    if len(clip_similarities.shape) == 2:
                        clip_similarities = (text_weight * clip_similarities + visual_weight * visual_clip_similarities.unsqueeze(0)) / total_weight
                    else:
                        clip_similarities = (text_weight * clip_similarities + visual_weight * visual_clip_similarities) / total_weight
                else:
                    clip_similarities = visual_clip_similarities
        
        if self.use_text and sbert_similarities is not None:
            similarities = clip_similarities + sbert_similarities
        else:
            similarities = clip_similarities
