...
from vipscene.constants import HOLODECK_BASE_DATA_DIR, DEBUGGING
import json
...
class FloorPlanGenerator:
    ...
    def generate_rooms(self, scene, plan_json, additional_requirements="N/A", visualize=False):
        ...
        rooms = self.get_plan(scene["query"], plan_json, scene["raw_floor_plan"], visualize)
        return rooms

    def get_plan(self, query, plan_json, raw_plan, visualize=False):
        parsed_plan = self.parse_raw_plan(plan_json, raw_plan)
    ...
    def parse_raw_plan(self, plan_json, raw_plan):
        parsed_plan = []
        room_types = []
        plans = [plan.lower() for plan in raw_plan.split("\n") if "|" in plan]
        for i, plan in enumerate(plans):
            room_type, floor_design, wall_design = plan.split("|")
            ...
            with open(plan_json, 'r') as json_file:
                ...
                new_width = plan_1['room_size'][0]
                new_length = plan_1['room_size'][1]
            vertices = [(0.0, 0.0), (0.0, new_length), (new_width, new_length), (new_width, 0.0)]
