...
from vipscene.generation.utils import *
...
class WallObjectGenerator:
    ...
    def generate_wall_objects_per_room(self, args):
        ...
        floor_object_name2id = {
            object["object_name"]: object["assetId"]
        ...
        # get constraints
        constraints_prompt = self.constraint_prompt_template.format(
            room_type=room_type,
        ...
        wall_objects_list = [
            (
                object_name,
                (
                    wall_object2dimension[object_name]["x"] * 100,
                    wall_object2dimension[object_name]["y"] * 100,
                    wall_object2dimension[object_name]["z"] * 100,
                ),
            )
    ...
    def parse_wall_object_constraints(
        ...
        for line in lines:
            ...
            if line[-1] == ".":
                line = line[:-1]  # remove the last period
            try:
            ...
            if object_name not in wall_object_names:
                continue

            try:
                target_floor_object_name = location.split(", ")[-1]
            except:
                print(f"Warning: cannot parse {location}.")
            ...
            if target_floor_object_name in floor_object_names:
                object2constraints[object_name] = {
                    "target_floor_object_name": target_floor_object_name,
                    "height": height,
                }
            else:
                object2constraints[object_name] = {
    ...
    def get_initial_state(self, scene, doors, windows, room_vertices, open_walls):
        ...
        for object in scene["floor_objects"]:
            ...
            if room_poly.contains(object_center):
                ...
                )  # the height should be twice the value of the y coordinate
                x_min, z_min, x_max, z_max = object_poly.bounds
                initial_state[object["object_name"]] = (
                    (x_min, 0, z_min),
                    (x_max, object_height, z_max),
                    object["rotation"]["y"],
                    object_vertices,
                    1,
                )
...
class DFS_Solver_Wall:
    ...
    def filter_collision(self, placed_objects, solutions):
        def intersect_3d(box1, box2):
            ...
            overlap_axes = [
                i for i in range(3)
                if box1['max'][i] >= box2['min'][i] and box1['min'][i] <= box2['max'][i]
            ]
            
            # If at least 2 axes overlap, check gap on non-overlapping axis
            if len(overlap_axes) >= 2:
                non_overlap_axes = [i for i in range(3) if i not in overlap_axes]
                for i in non_overlap_axes:
                    gap = max(box2['min'][i] - box1['max'][i], box1['min'][i] - box2['max'][i])
                    if gap > 20:
                        return False
                return True
            return False
        ...
        boxes = [
            ...
            for vertex_min, vertex_max, _, _, _ in placed_objects.values()
        ]

        return [
            solution for solution in solutions
            if not any(intersect_3d(box, {"min": solution[0], "max": solution[1]}) for box in boxes)
        ]
